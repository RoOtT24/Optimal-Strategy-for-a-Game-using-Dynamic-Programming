...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			//this.scene1 = firstScene();
			//this.scene2 = secondeScene();
			//this.scene3 = thirdScene();
...
//		labelTable.setLayoutX(5);
//		labelTable.setLayoutY(60);
//		labelTable.setFont(new Font(15));
		
		String[][] s = new String[moves.length][moves.length] ;
		for(int i = 0 ; i < moves.length ; ++i) {
			for(int j = 0; j < moves[i].length ; ++j) {
				s[i][j] = moves[i][j].tableString() ;
			}
		}
...
//		buttonNext.setLayoutX(1100);
//		buttonNext.setLayoutY(100);
//		buttonNext.setMinSize(150, 30);
//		buttonNext.setFont(new Font(STYLESHEET_CASPIAN, 20));
//		
//		buttonNext.setOnAction(new EventHandler<ActionEvent>() {
//			
//			@Override
//			public void handle(ActionEvent event) {
//				moves = findMoves(arr); // here
//				++i ;
//				++l ; 
//				
//			}
		...
		buttonStart.setOnAction(e -> {
			...
			if(arr.size()%2 ==0 && arr.size()>0) {
		        ...
		        for(int i=0; i < arr.size(); i++){
		            moves[i][i].first = arr.get(i);
		            //to track the sequence of moves
		            moves[i][i].index = i;
		        }
		        i = 0 ; l = 0 ;moves = findMoves(arr) ; 
				///////////////////////////
				//Pair[][] moves = new Pair[arr.size()][arr.size()];
	...
	public void printSequence(ArrayList<Integer> arr, Pair moves[][]){
        ...
        if(k < arr.size()) { // here
            ...
            if(k%2 == 0) {
            	user=1;
            	counter1+=temp ;
            	sb1.append("user"+user+ " => value: " + arr.get(step) + " " + "index: " + step + " \n\n");
            }
            else {
            	counter2 +=temp ;
            	user=2;
            	sb2.append("user"+user+ " => value: " + arr.get(step) + " " + "index: " + step + " \n\n");
            }
       ...
       //System.out.println(sb1.append("#").append(sb2));
